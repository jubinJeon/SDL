...
export default ({history}) => {
    ...
    useEffect(() => {
        ...
        if( !loading && data!= null && data.code ) {
            if(data.data.mbrAuthChnlCd != "SDL") {
                // SNS 회원
                setIsSns(true)
            }
            else
    ...
    const onHandleChange = () => {
        if(!isSns && idRef.current.value != '' && checkRef.current.checked
            || isSns && checkRef.current.checked) setActive(true)
    ...
    const DoUnregister = () => {
        ...
        else if( !isSns && idRef.current.value != data.data.mbrId ) {
            dispatch({type : 'TOAST', payload : {show : true , data : {msg: '이메일 주소가 잘못되었습니다.', code : '', dispatch : dispatch} , callback : toastCallback}})
            return ;
        }
        else if( !checkRef.current.checked ) {
            dispatch({type : 'TOAST', payload : {show : true , data : {msg: '회원 탈퇴 동의에 체크해야 합니다.', code : '', dispatch : dispatch} , callback : toastCallback}})
            return ;
        }
        ...
        .then((data) => {
            ...
            if(data.data.returnCode == "1") {
            ...
            else if(data.data.returnCode == "0" /* 잔고 없음 */ || data.data.returnCode == "2"/* 비회원 */ ) {
                ...
                handleClickOpen({
                    ...
                    handleComfirm: (()=> {
                        ...
                        // 제로페이 회원 탈퇴
                        API.unregisterZeroPay()
                        .then((data) => {
                            console.log(data)
                            if(data.data.returnCode == "1" /* 성공 */|| data.data.returnCode == "2" /* 비회원 */) {
                                console.log(mbrId)
                                ...
                                .catch((err) => { 
                                    dispatch({type : 'TOAST', payload : {show : true , data : {msg: '슬배생 회원탈퇴 실패.', code : '', dispatch : dispatch} , callback : toastCallback}})
                                })
                            }
                            else {
                                dispatch({type : 'TOAST', payload : {show : true , data : {msg: '제로페이 회원 탈퇴 실패', code : '', dispatch : dispatch} , callback : toastCallback}})
                            }
                        })
