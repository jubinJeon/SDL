    ...
    const register = () => {
        ...
        .then((data) => {
                ...
                API.register(idRef.current.value,
                    ...
                    "1").then((data) => {
                        console.log(data)
                        if(data.code) history.replace({pathname:ACTION.LINK_MY_SDL, popupTxt: '로그인 되었습니다.'})
                    })
                    .catch((err) => {
                        if(err.response.status === 400)
                            dispatch({type : 'TOAST', payload : {show : true , data : {msg: err.response.data.msg.split('|')[1], code : '', dispatch : dispatch} , callback : toastCallback}})
                        else {
